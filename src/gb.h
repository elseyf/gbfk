...
#define MAX_BYTES_PER_VBLANK 20
...
inline void enable_int(uint8_t _int){*reg(REG_INT) |= _int;}
inline void disable_int(uint8_t _int){*reg(REG_INT) &= ~(_int);}
...
inline void set_bg_pal(uint8_t _data){*reg(REG_BGP) = _data;}
inline void set_obj_pal0(uint8_t _data){*reg(REG_OBP0) = _data;}
inline void set_obj_pal1(uint8_t _data){*reg(REG_OBP1) = _data;}
